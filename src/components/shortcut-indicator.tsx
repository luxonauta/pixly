  ...
  return (
    <Transition appear show={isVisible}>
      <div className="fixed bottom-4 right-4 z-50">
        ...
        >
          <div className="flex items-center gap-2 rounded-lg bg-black/90 px-3 py-2 text-sm text-white backdrop-blur-sm">
            {currentShortcut.keys.map((key, index) => (
              <React.Fragment key={key}>
                <span className="inline-block rounded border border-white/20 bg-white/10 px-2 py-0.5 font-medium">
                  {key}
                </span>
